...
class GnssConvertisser():
    datum = None # [48.78458579360865, 11.473190410895723, 320] #latitude, longitude, ENU yaw (see geonav_transform repo)
    initiated_datum = False

    def __init__(self):
        rospy.init_node('gnss_convertisser')
        self.input_datum = rospy.get_param('~input_datum')
        self.topic_datum = rospy.get_param('~topic_datum')
        self.datum = rospy.get_param('~datum')

        #print(self.input_datum, self.topic_datum, self.datum)

        if self.input_datum == "topic":
          rospy.loginfo("GNSS Converter: use topic as source for datum")
          rospy.Subscriber(self.topic_datum, NavSatFix, self.callbackDatum)
        else:
          rospy.loginfo("Obtained Datum: %f, %f, %f", self.datum[0], self.datum[1], self.datum[2])
          self.initiated_datum = True
        
        #s = rospy.Service('gnss_convert', ConversionRequest, self.handleTransformRequest)


    def callbackDatum(self, data):
      self.datum = [data.latitude, data.longitude, data.altitude]

      if not self.initiated_datum:
        rospy.loginfo("Obtained Datum: %f, %f, %f", self.datum[0], self.datum[1], self.datum[2])
        self.initiated_datum = True


    def handleTransformRequest(self, req):
        
        answer = Odometry()

        if req.request_type == "LL2XY":
            #Convert GNSS pose -> map pose
            latitude = req.odom_request.pose.pose.position.y
            longitude = req.odom_request.pose.pose.position.x
            altitude = req.odom_request.pose.pose.position.z

            xg2, yg2 = ll2xy(latitude, longitude, self.datum[0], self.datum[1])

            #Answer with converted values
            answer.header.frame_id = req.odom_request.child_frame_id
            answer.pose = req.odom_request.pose
            answer.pose.pose.position.x = xg2
            answer.pose.pose.position.y = yg2
            answer.twist = req.odom_request.twist
        
        elif req.request_type == "XY2LL":
            #Convert map pose to gnss
            latitude = req.odom_request.pose.pose.position.y
            longitude = req.odom_request.pose.pose.position.x
            # altitude = req.odom_request.pose.pose.position.z

            longit, lat = xy2ll(req.odom_request.pose.pose.position.x, \
                                req.odom_request.pose.pose.position.y, \
                                self.datum[0], \
                                self.datum[1])
            ...
            answer.header.frame_id = req.odom_request.child_frame_id
            answer.pose = req.odom_request.pose
            answer.pose.pose.position.x = longit
            answer.pose.pose.position.y = lat
            answer.twist = req.odom_request.twist
...
if __name__ == '__main__':
    gnssConvertisser = GnssConvertisser()
