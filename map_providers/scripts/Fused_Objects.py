...
# Convert local coordinates to latitude and longitude
        ...
        objectAngle = atan2(pose_y, pose_x) * 180 / pi

        #objectAngle = objectAziDegree

        if objectAngle < 0:
          objectAziRad = (360 + objectAngle) * math.pi / 180
        elif objectAngle > 360:
          objectAziRad = (objectAngle - 360) * math.pi / 180
        else:

          objectAziRad = objectAngle * math.pi / 180
          objectAngle = objectAngle

        objectEuclideanDistance = math.sqrt(pow(pose_x, 2) + pow(pose_y, 2))
        objectGeographicalDistance = objectEuclideanDistance * abs(math.cos(0))


        objectLatRad = asin(sin(ref_lat) * cos(objectGeographicalDistance / m_radiusEarth) +
                            cos(ref_lat) * sin(objectGeographicalDistance / m_radiusEarth) *
                            cos(objectAziRad));
        objectLonRad = ref_lon + atan2(sin(objectAziRad) * sin(objectGeographicalDistance / m_radiusEarth) *
                                           cos(ref_lat), cos(objectGeographicalDistance / m_radiusEarth) -
                                                             sin(ref_lat) * sin(ref_lat));

        

        objectLatDegree = objectLatRad * (180 / pi);
        objectLonDegree = objectLonRad * (180 / pi);

        print('pakka_convert_lat', objectLatDegree )
        print('pakka_convert_lon', objectLonDegree )

        re_reconverted_x_y = ll2xy(lon=objectLonDegree, lat=objectLatDegree, origin_lat=location_pos.lat / 10000000.0, origin_lon=location_pos.lon / 10000000.0)
        #print('re_reconverted', re_reconverted_x_y)
        #print('original_pose', pose_x, pose_y)
        #print('car_pose', location_pos.lat / 10000000.0, location_pos.lon / 10000000.0)

        # Reference point (car_lat, car_lon)
        car_lat, car_lon = ego_lat, ego_lon # Example reference point (Los Angeles)
        print('car_latandlon', car_lat, car_lon)

# Object point (obj_lat, obj_lon)
        obj_lat, obj_lon = objectLatRad, objectLatRad  # Example object point
        #print('object_lat_and_lon', , )

# Altitude is assumed to be zero for simplicity, adjust as needed
        car_ecef = geodetic_to_ecef(car_lat, car_lon, 0)
        obj_ecef = geodetic_to_ecef(obj_lat, obj_lon, 0)
        print('after some conversion car',car_ecef)
        print('after some conversion object',obj_ecef)

# Convert ECEF to local Cartesian coordinates
        local_coords = ecef_to_local(obj_ecef, car_ecef)

        print("Local Cartesian Coordinates (x, y, z):", local_coords)

        print('x_new', local_coords[0]*100)

        pose_x_ego_latLon=xy2ll(x=local_coords[0]*100, y=local_coords[1]*100, orglat=ego_lat * 180/pi, orglon=ego_lon * 180/pi)
        print('verify local', pose_x_ego_latLon)
        ...
        mast_proj = Proj(proj='utm', zone=33, ellps='WGS84', units='m')
        ego_proj = Proj(proj='utm', zone=33, ellps='WGS84', units='m')
        print('degree or radian',mast_proj, ego_proj)
        
        # Transform mast coordinates to UTM
        mast_utm_x, mast_utm_y = transform(mast_proj, ego_proj, ref_pos_lon, ref_pos_lat)
        print('mast_valuesafter utm', mast_utm_x, mast_utm_y)
        ...
        # Transform ego coordinates to UTM
        ego_utm_x, ego_utm_y = transform(mast_proj, ego_proj, ego_lon, ego_lat)
        print('ego_valuesafter utm', ego_utm_x, ego_utm_y)
        ...
        pose_x_ego=pose_x-ego_x
        pose_y_ego=pose_y-ego_y

        print('final_x',pose_x_ego)
        print('final_y',pose_y_ego)

        pose_x_ego_latLon=xy2ll(x=pose_x_ego, y=pose_y_ego, orglat=ego_lat, orglon=ego_lon)
        print('converted_obj_lat and lon', pose_x_ego_latLon)
        '''
        ...
        POM.object_movement.position.x=re_reconverted_x_y[0]
